...
let computerDeck,
  ...
  playerCard,
  computerFirstCard,
  computerFirstCardDOM = null,
  isPlayerAceDealt,
  isComputerAceDealt,
  isDoubleOn,
  isSplitOn,
  isCleanSplittedRound,
  isSplitHandlerClicked;
let currentCardCount, currentCardSlot;

let playerValue = 0,
  computerValue = 0,
  playerBankroll = money.dataset.money,
  bet = 10,
  prevCard,
  nextCard,
  splitIndex = 0;
...
var playerBoards = [],
  playerCounts = [],
  playerValues = []; //array of boards -- to use in case of splits

//counter for basic strategy tracking
let rightPlay = 0,
  wrongPlay = 0,
  flippedComputerCard = 0;
...
function standHandler(event) {
  ...
  if (!isSplitOn) {
    ...
    if (playerBoards.length - splitIndex == 1) {
      isSplitOn = false;
      isCleanSplittedRound = true;
    }
  }
}
...
function splitHandler(event) {
  ...
  if (nextCard.cardToReturn != prevCard.cardToReturn) {
...
function flipPlayerCard() {
  ...
  if (CARD_VALUE_MAP[playerCard.value] == 11) {
    isPlayerAceDealt = true;
  }

  if (isPlayerAceDealt && playerValue > 21) {
    playerValue -= 10;
  ...
  if (playerValue > 21) {
    if (isSplitOn) {
      ...
      if (playerBoards.length - splitIndex == 1) {
        ...
        startComputerRound();
        return;
      } //lazy implementation, it would be
  ...
  if (isDoubleOn) startComputerRound();
...
function flipComputerCard() {
  ...
  if (CARD_VALUE_MAP[computerCard.value] == 11) {
    isComputerAceDealt = true;
  }

  if (isComputerAceDealt && computerValue > 21) {
    computerValue -= 10;
...
function flipComputerFirstCard() {
  ...
  computerFirstCardDOM = computerCardSlot.appendChild(
    computerFirstCard.getBlankHTML()
  ); //adding the first flipped card
  computerValue += CARD_VALUE_MAP[computerFirstCard.value];
  if (CARD_VALUE_MAP[computerFirstCard.value] == 11) {
    isComputerAceDealt = true;
  }
...
function setupRound() {
  ...
  if (
    CARD_VALUE_MAP[prevCard.cardToReturn] ==
    CARD_VALUE_MAP[nextCard.cardToReturn]
  ) {
    /* currentCardCount.innerText=playerValue/2; */ isSplitOn = true;
  }
...
function startComputerRound() {
  ...
  while (computerValue < 17) {
    //it reaches this point only if the player is standing
